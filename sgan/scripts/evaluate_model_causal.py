...
from sgan.models_causal_opt import TrajectoryGenerator
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            (obs_traj, pred_traj_gt, obs_traj_rel, pred_traj_gt_rel,
             non_linear_ped, loss_mask, traj_causal_inf_cvar1, traj_causal_inf_cvar2, traj_causal_inf_cvar3, traj_cvar1, traj_cvar2, traj_cvar3,seq_start_end) = batch # gt for ground truth
            ...
            for _ in range(num_samples): # num_samples = 20 cz best_k = 20, then take the min in helper over the 20 runs
                pred_traj_fake_rel = generator(
                    obs_traj, obs_traj_rel, traj_causal_inf, traj_cvars, seq_start_end
        ...
        if (save):
            torch.save(traj_pred_gt, 'THOR_full_test_8ts_causal_pred_gt.pt')
            torch.save(traj_pred_est, 'THOR_full_test_8ts_causal_pred_est.pt')
            torch.save(traj_obs, 'THOR_full_test_obs_8ts_causal_gt.pt')

...
def seq_collate(data):
	(obs_seq_list, pred_seq_list, obs_seq_rel_list, pred_seq_rel_list, non_linear_ped_list, loss_mask_list, traj_causal_inf_cvar1, traj_causal_inf_cvar2, traj_causal_inf_cvar3) = zip(*data)
	...
	out = [
	    ...
	    loss_mask, traj_causal_inf_cvar1, traj_causal_inf_cvar2, traj_causal_inf_cvar3, seq_start_end]
...
class TrajectoryDataset(Dataset): # A custom Dataset class must implement three functions: __init__, __len__, and __getitem__.
	...
	def __getitem__(self, index): # The __getitem__ function loads and returns a sample from the dataset at the given index idx. called internally when we create an instance of the class and pass an index to it
		...
		out = [
		    ...
		    self.non_linear_ped[start:end], self.loss_mask[start:end, :],  self.traj_causal_inf_cvar1[start:end, :], self.traj_causal_inf_cvar2[start:end, :], self.traj_causal_inf_cvar3[start:end, :]]

...
class PoolHiddenNet_g(nn.Module): # pooling hidden state of all people present in scene. encoder output and decoder output
    ...
    def __init__(
        ...
        self.mlp_pre_dim =   embedding_dim + h_dim  + ((self.obs_len) * self.embedding_dim_cvar1) + ((self.obs_len-1) * self.embedding_dim_cvar2) + ((self.obs_len-1) * self.embedding_dim_cvar3)# 1st mlp in fig 3 (pooling module)
    ...
    #@profile
    def forward(self, h_states, seq_start_end, end_pos, traj, traj_causal_inf, traj_cvars, mlp_pre_pool_dim_0):
        """
        Inputs:
...
class TrajectoryGenerator(nn.Module):
    ...
    def forward(self, traj, traj_rel, traj_causal_inf, traj_cvars, seq_start_end, user_noise=None):
        """
        Inputs:
        - obs_traj: Tensor of shape (obs_len, batch, 2)
        ...
        if self.pooling_type:
            end_pos = obs_traj[-1, :, :]
            pool_h = self.pool_net_g(final_encoder_h, seq_start_end, end_pos, traj, traj_causal_inf, traj_cvars, mlp_pre_pool_dim_0) # pool hidden state of encoder
...
class TrajectoryDiscriminator(nn.Module):
    ...
    def forward(self, traj, traj_rel, traj_causal_inf, traj_cvars, seq_start_end=None):
        """
        Inputs:
